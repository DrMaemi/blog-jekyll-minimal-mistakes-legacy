...
또, 배치 처리를 통한 문제 해결 시 고려해야 될 문제로 배치 사이즈를 어떻게 측정하는가에 대해 제대로 된 테스트를 진행하지 못했습니다. 배치 사이즈와 성능 향상은 서로 비례하는데 지나치게 큰 배치 사이즈는 런타임 중 OOM(Out Of Memory) 에러를 발생시키기 때문에 별도 테스트나 배치 사이즈를 결정하는 알고리즘이 필요한데 이런 부분에 대한 고도화를 진행하진 못했습니다. 다양한 도메인의 어플리케이션에서 배치 처리는 비즈니스 로직 상 대규모 데이터 처리에 대해 효율성과 단순성을 증가시키고 이에 따라 데이터 품질 향상과 더 빠른 비즈니스 인텔리전스가 가능하기에, Spring과 같은 어플리케이션 개발 프레임워크에서는 배치 처리를 전문적으로 할 수 있는 Spring Batch와 같은 프레임워크를 별도로 제공합니다. 배치 처리가 중요한 만큼 기회가 된다면 위 문제에 대한 고찰과 토이 프로젝트를 할 수 있으면 좋을 것 같습니다.
